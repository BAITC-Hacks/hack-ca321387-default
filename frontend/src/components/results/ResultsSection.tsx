...
export function ResultsSection({ response, query, pending, error, onRetry, onFocusField, onDateSelect }: Props) {
  ...
  return <section className="results-section" aria-live="polite" aria-labelledby="results-title">
    <div className="results-heading"><p className="eyebrow">02 / Результаты поиска</p>
      ...
      <p>{response.status === 'matched' ? count < 3 ? 'Все показанные подрядчики свободны на выбранную дату. Другие не прошли обязательные условия.' : 'Все показанные подрядчики свободны на выбранную дату и прошли обязательные условия.' : response.status === 'category_not_found' ? 'В выбранном городе нет подрядчиков этой категории. Измените город или категорию.' : 'Посмотрите, на каком условии закончились кандидаты.'}</p>
    </div>
